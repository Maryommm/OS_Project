...
public class OutputManager {
    ...
    //display what was computed
    public void printFinalOutput(
            // Lists are initialized in Scheduler and filled during execution:
            ArrayList<PCB> processes,
            ArrayList<GanttEntry> ganttChart,
            ArrayList<PCB> starvedProcesses,
            boolean isPriority//to know if used prio or no
    ) {
        //The results already exist (burst, start, termination), but waiting time and turnaround time need to be computed from them before printing.
        // Create MetricsCalculator object to compute waiting and turnaround times
        MetricsCalculator calculator = new MetricsCalculator();

        calculator.calculateProcessMetrics(processes);

        printGanttChart(ganttChart);//both grant and grant in detail
        printProcessTable(processes);//result table
    ...
    public void printGanttChart(ArrayList<GanttEntry> ganttChart) {
       ...
       //loop thru each entry(obj in gant list)
        for (GanttEntry entry : ganttChart) {
            System.out.print("| P" + entry.getProcessID() + " ");// output | P1 | P2 | P1 |
        }
        System.out.println("|");//for last 

        System.out.print(ganttChart.get(0).getStartTime());//start time

        for (GanttEntry entry : ganttChart) {
            System.out.print("    " + entry.getEndTime());//| P1 | P2 | P1 |
                                                          //0    5    10   15
        }
    ...
    }//Locale.US ensures decimals appear with a dot:
//only in prio cuz processes with low prio might wait indefinetily if higher keep being selected
    //SJF shortest first//RR everyone will take turn no starve
